"""
ERCOT GIS Report generator-interconnection-queue milestone history
(reportTypeId 15933).

Retains source documents, their publication metadata and exact bytes, and each
version of extracted project observations. ercot_gis_snapshots is maintained as
the latest-filing-per-month projection for existing timeline/dashboard readers.
Same-month corrections, changed bytes under an existing document ID, and parser
revisions retain their previous observations.

Ported 2026-07-17 from
interconnection-queue-tracker/services/fetcher/backfill_gis.py so the
standalone large-load-tracker product doesn't depend on that repo's stack.
The tracker's own copy is left alone (still serves its existing page); this
is a parallel, independent ingest into kardashev-data.

Usage:
    python -m ingest.ercot_gis --backfill          # all available months (one-time)
    python -m ingest.ercot_gis                     # check for new or revised source files
    python -m ingest.ercot_gis --dry-run [--backfill]
"""
from __future__ import annotations

import argparse
import hashlib
import io
import logging
import re
import sys
import time
from datetime import datetime

import pandas as pd
import requests

log = logging.getLogger(__name__)

REPORT_TYPE_ID = 15933
PARSER_VERSION = "1"
DOWNLOAD_URL = "https://www.ercot.com/misdownload/servlets/mirDownload"

# ERCOT renamed this sheet at some point ("Project Details" -> "Project
# Details - Large Gen") and the header row position drifts month to month
# (variable-length disclaimer/footnote text above it shifts everything down).
# So: try each known sheet name, but locate the header row dynamically by
# scanning for the row whose first cell is literally "INR", rather than
# assuming a fixed skiprows offset.
SHEET_NAMES = ["Project Details - Large Gen", "Project Details"]

COLUMN_ALIASES = {
    "GINR Study Phase": "GIM Study Phase",
}

COLS = [
    "queue_id", "snapshot_month", "project_name", "gim_study_phase", "county",
    "zone", "projected_cod", "fuel", "technology", "capacity_mw",
    "screening_study_started", "screening_study_complete", "ia_signed",
    "construction_start", "construction_end", "approved_for_energization",
    "approved_for_synchronization", "poi_location",
]


def find_header_row(raw: pd.DataFrame, max_scan: int = 60) -> int | None:
    for i in range(min(max_scan, len(raw))):
        if str(raw.iat[i, 0]).strip() == "INR":
            return i
    return None


def list_gis_docs() -> list[dict]:
    listing = requests.get(
        "https://www.ercot.com/misapp/servlets/IceDocListJsonWS",
        params={"reportTypeId": REPORT_TYPE_ID}, timeout=60,
    )
    listing.raise_for_status()
    docs = listing.json()["ListDocsByRptTypeRes"]["DocumentList"]
    return [d["Document"] for d in docs
            if str(d["Document"].get("FriendlyName", "")).startswith("GIS_Report")]


def snapshot_month_from_name(name: str) -> str:
    # "GIS_Report_April_2020" / "GIS_Report_Jun2026" -> "2020-04" / "2026-06"
    m = re.search(r"GIS_Report_?([A-Za-z]+)_?(\d{4})", name)
    if not m:
        return name
    month_str, year = m.group(1), int(m.group(2))
    try:
        month = datetime.strptime(month_str[:3], "%b").month
    except ValueError:
        return name
    return f"{year:04d}-{month:02d}"


def download_document(doc: dict) -> bytes:
    resp = requests.get(DOWNLOAD_URL, params={"doclookupId": doc["DocID"]}, timeout=180)
    resp.raise_for_status()
    return resp.content


def publication_time(doc: dict) -> datetime:
    # ERCOT timestamps without offsets are local Texas wall-clock times.
    from zoneinfo import ZoneInfo

    value = pd.Timestamp(doc["PublishDate"])
    if pd.isna(value):
        raise ValueError("GIS filing has no publication date")
    if value.tzinfo is None:
        value = value.tz_localize(ZoneInfo("America/Chicago"), ambiguous="raise", nonexistent="raise")
    return value.to_pydatetime()


def parse_document(content: bytes, source_name: str) -> pd.DataFrame | None:
    for sheet in SHEET_NAMES:
        try:
            raw = pd.read_excel(io.BytesIO(content), sheet_name=sheet, header=None)
        except Exception:
            continue
        header_row = find_header_row(raw)
        if header_row is None:
            continue
        header = raw.iloc[header_row].tolist()
        data = raw.iloc[header_row + 1:].reset_index(drop=True)
        data.columns = header
        data = data.rename(columns=COLUMN_ALIASES)
        # blank separator row(s) between header and real data, then rows with
        # no queue ID at all (end of table / footnotes) -- drop both
        data = data.dropna(subset=["INR"]) if "INR" in data.columns else data
        if "INR" in data.columns and len(data) > 0:
            return data

    log.warning("SKIP %s: no header row found", source_name)
    return None


def to_rows(df: pd.DataFrame, snapshot_month: str) -> list[tuple]:
    out = []
    df = df.astype(object).where(pd.notna(df), None)
    for _, r in df.iterrows():
        qid = r.get("INR")
        if pd.isna(qid) or str(qid).strip() == "":
            continue
        mw = r.get("Capacity (MW)")
        try:
            mw = float(mw) if pd.notna(mw) else None
        except (TypeError, ValueError):
            mw = None
        poi = r.get("POI Location")
        if poi is not None and str(poi).strip() in {"", "nan", "None"}:
            poi = None
        elif poi is not None:
            poi = str(poi).strip()
        out.append((
            str(qid).strip(), snapshot_month,
            r.get("Project Name"), r.get("GIM Study Phase"), r.get("County"),
            r.get("CDR Reporting Zone"), r.get("Projected COD"), r.get("Fuel"),
            r.get("Technology"), mw,
            r.get("Screening Study Started"), r.get("Screening Study Complete"),
            r.get("IA Signed"), r.get("Construction Start"), r.get("Construction End"),
            r.get("Approved for Energization"), r.get("Approved for Synchronization"),
            poi,
        ))
    return out


def ingest_ercot_gis(full_backfill: bool = False, dry_run: bool = False, limit: int | None = None) -> int:
    """Check each available source file, preserving revised files and re-parses.

    Downloading is intentional: a document ID can be reused with changed bytes.
    Identical document/date/hash/parser versions are skipped before parsing.
    --backfill is retained for CLI compatibility; both modes discover all files.
    """
    from ingest.writer import existing_ercot_gis_filings, save_ercot_gis_filing

    known = set() if dry_run else existing_ercot_gis_filings()
    docs = sorted(list_gis_docs(), key=lambda d: (publication_time(d), str(d["DocID"])))
    if limit is not None:
        docs = docs[:limit]
    log.info("Checking %d GIS_Report documents (backfill=%s)", len(docs), full_backfill)

    total_rows = 0
    for i, doc in enumerate(docs):
        name = doc["FriendlyName"]
        month = snapshot_month_from_name(name)
        if not re.fullmatch(r"\d{4}-(0[1-9]|1[0-2])", month):
            raise ValueError(f"Unrecognized GIS report month: {name}")
        published_at = publication_time(doc)
        content = download_document(doc)
        digest = hashlib.sha256(content).hexdigest()
        key = (str(doc["DocID"]), published_at, digest, PARSER_VERSION)
        time.sleep(1)  # Every download, including unchanged files, is throttled.
        if key in known:
            continue
        log.info("[%d/%d] %s -> %s", i + 1, len(docs), name, month)
        df = parse_document(content, name)
        if df is None:
            raise ValueError(f"No project sheet found in GIS filing: {name}")
        rows = to_rows(df, month)
        if not rows:
            raise ValueError(f"Empty GIS filing: {name}; refusing to replace monthly projection")
        log.info("  %d projects", len(rows))
        if not dry_run:
            total_rows += save_ercot_gis_filing(
                source_document_id=str(doc["DocID"]),
                source_url=f"{DOWNLOAD_URL}?doclookupId={doc['DocID']}",
                source_name=name, published_at=published_at, snapshot_month=month,
                content_sha256=digest, raw_file=content, parser_version=PARSER_VERSION, rows=rows,
            )
            known.add(key)
        else:
            total_rows += len(rows)
    log.info("done: %d new observation rows%s", total_rows, " (dry run, not written)" if dry_run else "")
    return total_rows


if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(name)s: %(message)s", stream=sys.stdout)
    ap = argparse.ArgumentParser()
    ap.add_argument("--backfill", action="store_true", help="check all available source files (also the default)")
    ap.add_argument("--dry-run", action="store_true")
    ap.add_argument("--limit", type=int, default=None, help="only process N docs (testing)")
    args = ap.parse_args()
    ingest_ercot_gis(full_backfill=args.backfill, dry_run=args.dry_run, limit=args.limit)
